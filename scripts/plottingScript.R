...
# Folder holding the new bowhead per-replicate files
bow_dir <- "bowheadWhale_080626/"   # <- change if the new run is elsewhere

input_files <- Sys.glob(file.path(bow_dir, "*_with_distances.tsv"))

stopifnot(length(input_files) > 0)   # fails loudly if the glob matches nothing
...
summary_table <- map_dfr(input_files, function(file) {
  ...
  tibble(
    file = basename(file),
    mean_branch_distance   = mean(data$branch_distance, na.rm = TRUE),
    median_branch_distance = median(data$branch_distance, na.rm = TRUE)
  )
})

write_tsv(summary_table, file.path(bow_dir, "summary_branch_distances.tsv"))
...
plot_simulation_summary <- function(species_name, filepath, method_labels = NULL, extended_species = FALSE) {
  ...
  regex_pattern <- if (extended_species) {
    "^([A-Za-z0-9]+)_([A-Za-z]+)_([0-9]+)x_([0-9]+)([A-Za-z]+)_.*Result([0-9]+)$"
  } else {
...
bow_plot <- plot_simulation_summary(
  ...
  filepath = "bowheadWhale_080626/summary_branch_distances.tsv", extended_species = TRUE
)
bow_plot
...
# Filepaths
files <- list(
  Beluga       = "BelugaSimRes/summary_branch_distances.tsv",
  HarpSeal     = "harpSealSimRes/summary_branch_distances.tsv",
  Narwhal      = "NarwhalSimRes/summary_branch_distances.tsv",
  BowheadWhale = "bowheadWhale_080626//summary_branch_distances.tsv"
)
...
read_simulation_file <- function(filepath, extended_species = FALSE) {
  ...
  # =============================================================================
  # BUILD summary_branch_distances.tsv FOR THE NEW BOWHEAD RUN
  # Each per-replicate *_with_distances.tsv has a `branch_distance` column.
  # We collapse each file to one row: file + mean + median.
  # =============================================================================
  ...
  # NOTE: the extended regex now allows an OPTIONAL middle section
  #       ([A-Za-z]+)(?:_.*)?Result  so it matches BOTH
  #         10HighResult10              (old style: harp seal, narwhal)
  #         10High_fin_1000Result10     (new style: bowhead)
  # =============================================================================
  ...
  plot_simulation_summary <- function(species_name, filepath, method_labels = NULL, extended_species = FALSE) {
    ...
    regex_pattern <- if (extended_species) {
      ...
      "^([A-Za-z]+)_([A-Za-z]+)_([0-9]+)x_([0-9]+)([A-Za-z]+)(?:_.*)?Result([0-9]+)$"
    }
  ...
  plot_simulation_summary <- function(
    ...
    # Select regex pattern (optional middle section, see note above)
    regex_pattern <- if (extended_species) {
      "^([A-Za-z0-9]+)_([A-Za-z]+)_([0-9]+)x_([0-9]+)([A-Za-z]+)(?:_.*)?Result([0-9]+)$"
    } else {
      "^([A-Za-z]+)_([A-Za-z]+)_([0-9]+)x_([0-9]+)([A-Za-z]+)(?:_.*)?Result([0-9]+)$"
  ...
  files <- list(
    ...
    BowheadWhale = list(path = "bowheadWhale_080626/summary_branch_distances.tsv", extended = TRUE)
  )
  ...
  tree <- read.tree("Narwhal.new.dnd")
  
  clade_info <- read_excel("ref_mitogenome_clade_info.xlsx") |>
    filter(species == "narwhal")
  ...
  heatmap_plot <- ggplot(df_plot, aes(x = Clade1, y = Clade2, fill = AvgDistance)) +
    ...
    labs(
      x = "narwhal haplogroups",
      y = "narwhal haplogroups"
    ) +
    theme_bw(base_size = 14) +
    theme(
  ...
  heatmap_plot
  ggsave("Narwhal_clade_distance_heatmap.png", heatmap_plot, width = 7, height = 6, dpi = 600)
  ...
  true_source_R1 <- c("DL0852", "DL1661")
  true_sources_R2 <- c("DL0852", "DL1661")
  clade_info <- read_excel("../ref_mitogenome_clade_info.xlsx") |>
    filter(species == "beluga")
  
  
  B50R1 <- read.csv("B50R1.txt", header = FALSE)
  B50R2 <- read.csv("B50R2.txt", header = FALSE, sep = "\t")
  B500R1 <- read.csv("B500R1.txt", header = FALSE)
  B500R2 <- read.csv("B500R2.txt", header = FALSE, sep = "\t")
  B100R1 <- read.csv("B100R1.txt", header = FALSE)
  B100R2 <- read.csv("B100R2.txt", header = FALSE, sep = "\t")
  B1000R1 <- read.csv("B1000R1.txt", header = FALSE)
  ...
  make_tree_plot <- function(tree, clade_info, true_sources, pred_sources, title = NULL) {
    ...
    # Plot
    p <- ggtree(tree) %<+% meta +
      geom_tree(aes(color = branch_color), linewidth = 1) +
  ...
  p_B50R1   <- make_tree_plot(tree, clade_info, true_source_R1, as.character(B50R1$V1), title = "Beluga 100 fragments - 1 source")
  p_B500R1  <- make_tree_plot(tree, clade_info, true_source_R1, as.character(B500R1$V1), title = "Beluga 1000 fragments - 1 source")
  p_B100R1  <- make_tree_plot(tree, clade_info, true_source_R1, as.character(B100R1$V1), title = "Beluga 200 fragments - 1 source")
  p_B1000R1 <- make_tree_plot(tree, clade_info, true_source_R1, as.character(B1000R1$V1), title = "Beluga 2000 fragments - 1 source")
  
  p_B50R2   <- make_tree_plot(tree, clade_info, true_sources_R2, as.character(B50R2$V1), title = "Beluga 100 fragments - 2 source")
  p_B500R2  <- make_tree_plot(tree, clade_info, true_sources_R2, as.character(B500R2$V1), title = "Beluga 1000 fragments - 2 source")
  p_B100R2  <- make_tree_plot(tree, clade_info, true_sources_R2, as.character(B100R2$V1), title = "Beluga 200 fragments - 2 source")
  p_B1000R2 <- make_tree_plot(tree, clade_info, true_sources_R2, as.character(B1000R2$V1), title = "Beluga 2000 fragments - 2 source")
  ...
  plot_R1
  ggsave("Beluga_2sources.png", plot = plot_R2, width = 13, height = 9, dpi = 600)
  ggsave("Beluga_1sources.png", plot = plot_R1, width = 13, height = 9, dpi = 600)
  
  # ── Panel identifiers ─────────────────────────────────────────────────────────
  # Build each row WITHOUT labels first (labels go on the combined figure)
  ...
  # Wrap in ggdraw and add a left margin so the label isn't clipped
  final_plot <- plot_grid(
    ggdraw(both_panels) +
      theme(plot.margin = margin(t = 5, r = 5, b = 5, l = 15)),  # l = extra left space
    shared_legend,
    ncol        = 1,
    rel_heights = c(1, 0.07)
  )
  final_plot
  ggsave("Beluga_combined.png", plot = final_plot, width = 15, height = 18, dpi = 600)
  df_raw <- read.csv(filepath, sep = '\t')
...
plot_R1
ggsave("Beluga_2sources.png", plot = plot_R2, width = 13, heigh = 9, dpi = 600)
ggsave("Beluga_1sources.png", plot = plot_R1, width = 13, heigh = 9, dpi = 600)
